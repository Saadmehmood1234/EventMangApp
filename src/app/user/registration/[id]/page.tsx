...
const EventRegistrationForm: React.FC<EventDetailProps> = ({ params }) => {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        if (statusCode === 400) {
          ...
          setApiResponseMessage("An error occurred during registration.");
        }
      } else {
        console.error("Unexpected Error:", error);
        setApiResponseMessage(
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gray-200 dark:bg-gray-900 p-6">
      ...
      {/* Form Card */}
      <div className="bg-gradient-to-r from-blue-300 to-indigo-300 border-2 border-gray-200/50 rounded-lg shadow-lg p-6 sm:p-8 w-full max-w-4xl">
        <div className="p-4 rounded-md mb-6 flex flex-row max-sm:flex-col justify-between border-b-4 border-[#cc2b50]">
          <div className="mb-4 lg:mb-0">
            <h2 className="text-2xl font-bold text-gray-200 mb-6">
              About This Event
          ...
          <div className="flex flex-col items-center lg:items-end">
            <Image
              src={currentUser.image || "https://i.pravatar.cc/150?img=5"}
              alt="User"
              className="w-36 h-36 rounded-full mb-3" // Adjusted for 30px dimensions
              width={100} // Optional: Set fixed width for better layout control
              height={100} // Optional: Set fixed height for better layout control
            />
            <span className="text-xl font-extrabold text-gray-200 text-center">
              {currentUser.name}
            </span>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-6">
          ...
          <div className="col-span-2 mt-4">
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 hover:bg-indigo-400 text-white font-bold py-2 px-4 rounded"
              disabled={loading}
            >
              {loading ? "Submitting..." : "Register"}
              {error}
