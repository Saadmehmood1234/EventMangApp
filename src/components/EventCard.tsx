...
const EventsCard: React.FC<EventsCardProps> = ({ events, searchQuery }) => {
  ...
  return (
    <div className="p-8 bg-gradient-to-b from-gray-800 to-gray-900 rounded-lg shadow-lg max-w-6xl mx-auto">
      ...
      {filteredEvents.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {visibleEvents.map((event) => (
            <Link key={event.id} href={`/user/events/${event.id}`}>
              <div className="bg-gray-700 rounded-lg overflow-hidden shadow-lg transform transition-all duration-300 hover:scale-105 cursor-pointer">
                <img
                  className="w-full h-48 object-cover"
                  src={event.image || "eventa1.jpg"}
                  alt={event.title}
